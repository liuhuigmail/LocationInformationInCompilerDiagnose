...
public class TargetXMLWriter extends AbstractXMLWriter {
    ...
    protected void serializeProcessors(ProcessorList actions, boolean preProcesses) {
        ...
        while (iter.hasNext()) {
            ...
            if (FileDumpProcessor.class.isAssignableFrom(pp.getClass())) {
                FileDumpProcessor pp1 = (FileDumpProcessor)pp;
				serializeProcessorHeader(XML_PROCESSOR_DUMP, preProcesses, pp1);
				sb.append(XMLTool.encodeProperty(XML_PP_DUMP_DIRECTORY, FileSystemManager.getAbsolutePath(pp1.getDestinationFolder())));
				sb.append(XMLTool.encodeProperty(XML_PP_DUMP_NAME, pp1.getReportName()));
				sb.append(XMLTool.encodeProperty(XML_PP_ADD_STATS, pp1.isAppendStatistics()));
				sb.append(XMLTool.encodeProperty(XML_PP_LIST_STORED_FILES, pp1.isAppendStoredFiles()));
				sb.append(XMLTool.encodeProperty(XML_PP_MAX_LISTED_FILES, pp1.getMaxStoredFiles()));
				sb.append("/>");
    ...
    protected void serializeProcessor(FileDumpProcessor pp, boolean postProcess) {
        sb.append("\n<");
		sb.append(XML_PROCESSOR_DUMP);
		sb.append(XMLTool.encodeProperty(XML_PP_AFTER, postProcess));
		
		sb.append(XMLTool.encodeProperty(XML_PP_RUN_SUCCESS, pp.isRunIfOK()));
		sb.append(XMLTool.encodeProperty(XML_PP_RUN_ERROR, pp.isRunIfError()));
		sb.append(XMLTool.encodeProperty(XML_PP_RUN_WARNING, pp.isRunIfWarning()));
		
		sb.append(XMLTool.encodeProperty(XML_PP_RUN_BACKUP, pp.isRunBackup()));
		sb.append(XMLTool.encodeProperty(XML_PP_RUN_MERGE, pp.isRunMerge()));
		sb.append(XMLTool.encodeProperty(XML_PP_RUN_CHECK, pp.isRunCheck()));
    ...
    protected void serializeProcessor(SendReportByMailProcessor pp, boolean postProcess) {
        sb.append("\n<");
		sb.append(XML_PROCESSOR_EMAIL_REPORT);
		sb.append(XMLTool.encodeProperty(XML_PP_AFTER, postProcess));
    ...
    protected void endMedium(AbstractIncrementalFileSystemMedium medium) {
		...
		EncryptionPolicy policy = medium.getEncryptionPolicy();
    ...
    protected void serializeHandler(DefaultArchiveHandler handler) {
        sb.append("\n<");
		sb.append(XML_HANDLER);
		sb.append(XMLTool.encodeProperty(XML_HANDLER_TYPE, XML_HANDLER_TYPE_STANDARD));
        sb.append("/>");
    }
    ...
    protected void serializeHandler(DeltaArchiveHandler handler) {
        sb.append("\n<");
		sb.append(XML_HANDLER);
		sb.append(XMLTool.encodeProperty(XML_HANDLER_TYPE, XML_HANDLER_TYPE_DELTA));
        sb.append("/>");
    }
