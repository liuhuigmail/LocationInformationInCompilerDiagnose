...
public class TargetXMLReader implements XMLTags {
	...
	protected Processor readDumpProcessor(Node node) throws AdapterException {
		...
		if (runIfErrorNode != null || runIfWarningNode != null || runIfOKNode != null) {
			pp.setRunIfError(runIfErrorNode != null && Boolean.valueOf(runIfErrorNode.getNodeValue()).booleanValue());
			pp.setRunIfWarning(runIfWarningNode != null && Boolean.valueOf(runIfWarningNode.getNodeValue()).booleanValue());
			pp.setRunIfOK(runIfOKNode != null && Boolean.valueOf(runIfOKNode.getNodeValue()).booleanValue());
		
			pp.setRunBackup(runBackupNode == null || Boolean.valueOf(runBackupNode.getNodeValue()).booleanValue());
			pp.setRunMerge(runMergeNode != null && Boolean.valueOf(runMergeNode.getNodeValue()).booleanValue());
			pp.setRunCheck(runCheckNode != null && Boolean.valueOf(runCheckNode.getNodeValue()).booleanValue());
		} else if (runSchemeNode != null) {
			// BACKWARD-COMPATIBILITY : version between 7.1.1 and 7.1.4 //
			if (XML_PP_RUN_SCHEME_ALWAYS.equals(runSchemeNode.getNodeValue())) {
				pp.setRunAlways();
			} else if (XML_PP_RUN_SCHEME_FAILURE.equals(runSchemeNode.getNodeValue())) {
				pp.setRunIfError(true);
				pp.setRunIfWarning(false);
				pp.setRunIfOK(false);
			} else if (XML_PP_RUN_SCHEME_SUCCESS.equals(runSchemeNode.getNodeValue())) {
				pp.setRunIfError(false);
				pp.setRunIfWarning(true);
				pp.setRunIfOK(true);
			} else {
				throw new AdapterException("Run rule not supported for processor " + pp.getName() + " : " + runSchemeNode.getNodeValue());
			}
			// EOF BACKWARD-COMPATIBILITY : version between 7.1.1 and 7.1.4 //
		} else {
			// BACKWARD-COMPATIBILITY : version older than 7.1.1 //
	...
	protected ArchiveMedium readMedium(Node mediumNode, AbstractTarget target) throws IOException, AdapterException, ApplicationException {
		...
		EncryptionPolicy encrArgs1 = new EncryptionPolicy();
		encrArgs1.setEncrypted(isEncrypted);
		encrArgs1.setEncryptionAlgorithm(encryptionAlgo);
		encrArgs1.setEncryptNames(encryptNames == null ? true : encryptNames.booleanValue());
		encrArgs1.setEncryptionKey(encryptionKey);
		encrArgs1.setNameWrappingMode(nameWrappingMode);
		...
		EncryptionPolicy encrArgs = encrArgs1;
	...
	protected DirectoryArchiveFilter readDirectoryArchiveFilter(Node filterNode) throws AdapterException {
		...
		if (directoryNode != null) {
			filter.acceptParameters(directoryNode.getNodeValue());
		}
