...
public class DeprecatedManifestAdapter implements ManifestReader {
	...
	public Manifest read(File file) throws AdapterException {
        try {
            if (FileSystemManager.exists(file)) {
				...
				if (index != -1) {
					String versionStr = content.substring(0, index);
					version = Long.parseLong(versionStr);
					content1 = content.substring(index + HEADER.length());
				}
				
				if (version == 0) {
					return decodeVersion0(content1);
				} else if (version == 1) {
					return decodeVersion1(content1);
				} else if (version == 2) {
					return decodeVersion2(content1);           
				} else {
	...
	private Manifest decode(String source) {
		...
		if (version == 0) {
			content = Util.replace(content, SEPARATOR, " " + SEPARATOR + " ");
