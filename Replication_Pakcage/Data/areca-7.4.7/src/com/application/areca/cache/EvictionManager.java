...
public class EvictionManager {
    ...
    public synchronized boolean gcIfNeeded() {    	
        if (! (OSTool.getMemoryUsage() < (1 - MemoryHelper.getMemorySafetyMargin()) * OSTool.getMaxMemory())) {
            this.gc();
            return false;
        } else {
            return true;
    ...
    private synchronized void gc() {
        ...
        while (! (OSTool.getMemoryUsage() < TARGET_CACHE_MEMORY_USAGE_RATIO * OSTool.getMaxMemory())) {
