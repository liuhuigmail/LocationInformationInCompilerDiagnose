...
public class ArchiveDataMap {
    ...
    public Object get(File key) {
        ...
        if (data == null) {
	        ...
	        String currentHash = "" + FileSystemManager.getInstance().getDriver(key).length(key) + "#" + FileSystemManager.lastModified(key);
    ...
    private static String computeHash(File file) {
        return "" + FileSystemManager.getInstance().getDriver(file).length(file) + "#" + FileSystemManager.lastModified(file);
    }
}