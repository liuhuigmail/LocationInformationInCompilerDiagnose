...
implements UserInformationChannel {
	...
	public void completionChanged(TaskMonitor task) {
        try {
			int pc = (int)(0.1*(100 * task.getGlobalCompletionRate()));
			if (pc != this.previousProgress) {
			    this.previousProgress = pc;
			    String info = "" + (10*pc) + "%";
			    if (this.displayThreadName) {
			        this.logger.info(info, Thread.currentThread().getName());
			        Logger.defaultLogger().info(info, Thread.currentThread().getName());
			    } else {
