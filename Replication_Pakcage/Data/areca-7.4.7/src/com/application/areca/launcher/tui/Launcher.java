...
implements CommandConstants {
				...
				public void run() {
					try {
						...
						try {
							...
							if (target.checkResumeSupported() == null && handler != null) {
								if (handler.shallSearchForPendingTransactions()) {
									...
									if (transactionPoint != null) {
										...
										if (handler.shallHandleTransactionPoint(transactionPoint)) {
											String tpVersion;
											try {
							...
							target.processBackup(manifest, backupScheme, checkParams, transactionPoint, buildContext(item));
						} finally {
	...
	throws Exception {
		...
		ProcessContext context1 = new ProcessContext((item instanceof AbstractTarget) ? (AbstractTarget)item : null, channel, new TaskMonitor("tui-main"));
		context1.getReport().setLogMessagesContainer(Logger.defaultLogger().getTlLogProcessor().activateMessageTracking());

		ProcessContext context = context1;
