...
public class FilterRepository {
	...
	public static boolean checkParameters(String params, int filterIndex) {
		...
		ArchiveFilter filter1 = null;
		Class cls = CLASSES[filterIndex];
		try {
			filter1 = (ArchiveFilter)cls.newInstance();
		} catch (InstantiationException e1) {
			Logger.defaultLogger().error("Error instanciating index " + filterIndex, e1);
		} catch (IllegalAccessException e2) {
			Logger.defaultLogger().error("Error instanciating index " + filterIndex, e2);
		}
		ArchiveFilter filter = filter1;
	...
	public static int getIndex(ArchiveFilter currentFilter) {
		...
		int index = DEFAULT_INDEX;
		for (int i=0; i<CLASSES.length; i++) {
			if (CLASSES[i].isAssignableFrom(currentFilter.getClass())) {
				index = i;
				break;
			}
		}
