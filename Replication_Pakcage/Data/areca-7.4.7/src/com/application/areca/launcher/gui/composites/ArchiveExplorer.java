...
implements MouseListener, Listener {
	...
	throws ApplicationException {
		...
		while (iter.hasNext()) {
			...
			final TreeItem item1 = chld;
			TraceEntry data = (TraceEntry) item1.getData();
			long length = 0;
			boolean stored = true;
			...
			if (stored) {
				item1.setForeground(Colors.C_BLACK);
			} else {
				item1.setForeground(Colors.C_LIGHT_GRAY);
			}
			
			if (data.getType() == MetadataConstants.T_SYMLINK || data.getType() == MetadataConstants.T_PIPE) {
				// SymLinks
				item1.setFont(deriveItalicFont(item1));
			}
			...
			if (
				...
				item1.setImage(ArecaImages.ICO_FS_FOLDER);
			} else if (data.getType() == MetadataConstants.T_PIPE) {
				item1.setImage(ArecaImages.ICO_FS_PIPE);
			} else {
				item1.setImage(ArecaImages.ICO_FS_FILE);
			}
			...
			item1.setText(0, label);
			if (((!stored) && (!displayNonStoredItemsSize))) {
				item1.setText(1, " ");
			} else {
				item1.setText(1, Utils.formatFileSize(length));
			}
			
			if (data.getType() == MetadataConstants.T_DIR) {
				new TreeItem(item1, ITEM_STYLE);
			}
		}
	}
	...
	public void mouseDown(MouseEvent e) {
		try {
			if (e.button == 3) {
				(logicalView ? Application.getInstance()
									.getArchiveContextMenuLogical() : Application.getInstance()
									.getArchiveContextMenu()).setVisible(true);
			}
