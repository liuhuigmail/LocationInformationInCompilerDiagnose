...
implements MouseListener, Listener, Refreshable { 
	...
	public void refresh() {
		...
		if (haveTargetsChanged()) {
			// REMOVE EXISTING
			Iterator chkIter1 = this.checkBoxes.iterator();
			while (chkIter1.hasNext()) {
				Button btn = (Button)chkIter1.next();
				btn.dispose();
			}
			this.checkBoxes.clear();
			this.targets.clear();
	...
	public void mouseDown(MouseEvent e) {
		...
		if (item != null && item.getParentItem() != null) {
			if (e.button == 3) {
				Application.getInstance().getSearchContextMenu().setVisible(true);
			}
		}
	}
