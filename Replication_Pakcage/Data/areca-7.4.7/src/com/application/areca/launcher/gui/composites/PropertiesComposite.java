...
extends Composite { 
    ...
    public void refresh() {
        ...
        if (application.isCurrentObjectTarget()) {
            ...
            TargetGroup group = application.getCurrentTargetGroup();
    ...
    private void fillData(TargetGroup group) {
        this.addProperty(RM.getLabel("property.element.label"), group.getName());
        final String conf = 
			group.getLoadedFrom() == null ? 
			FileSystemManager.getAbsolutePath(group.computeConfigurationFile(application.getWorkspace().getPathFile())) : 
			group.getLoadedFrom().toString();
		this.addProperty(RM.getLabel("property.conf.label"), conf);
		
		if (group.getLoadedFrom().isBackupCopy()) {
			this.addProperty(RM.getLabel("property.backup.label"), "true");
		}
    ...
    private void fillData(FileSystemTarget tg) {
        try {
			...
			item.setText(0, RM.getLabel("property.element.label") + " : ");
			item.setText(1, AbstractWindow.configureForTable(tgName));
            if (tg.getId() >= 0) {
