...
implements MouseListener, Refreshable, Listener { 
	...
	public void mouseDown(MouseEvent e) {
		...
		if (item != null) {
			if (e.button == 3) {
				Application.getInstance().getHistoryContextMenu().setVisible(true);
			}
		}
	}
		...
		if (e.widget instanceof Tree) {
			...
			if (selection.length == 1) {
				...
				if (data != null && data.getType() == MetadataConstants.T_FILE) {
					try {
						this.application.enableWaitCursor();
					
						EntryArchiveData[] currentEntryData = this.application.getCurrentTarget().getMedium().getHistory(data.getKey());
						this.resetHistoryContent();
					
						for (int i=currentEntryData.length - 1; i>=0; i--) {
							Manifest mf = currentEntryData[i].getManifest();                
							TableItem item1 = new TableItem(this.history, SWT.NONE);
					
							item1.setText(0, Application.STATUS_LABELS[currentEntryData[i].getStatus() + 1]); 
							item1.setImage(0, Application.STATUS_ICONS[currentEntryData[i].getStatus() + 1]); 
							item1.setData(currentEntryData[i]);
							if (mf != null) {
								item1.setText(3, Utils.formatDisplayDate(mf.getDate()));
							} else {
								item1.setText(3, " ");
							}  	
							if (currentEntryData[i].getHash() != null) {
								try {
									...
									item1.setText(2, Utils.formatDisplayDate(cal));
									
									long size = ArchiveTraceParser.extractFileSizeFromTrace(currentEntryData[i].getHash());
									item1.setText(1, Utils.formatFileSize(size));
								} catch (Exception e1) {
									Logger.defaultLogger().warn("Error reading data for " + data.getKey() + " : " + data.getData(), e1);
								}
							}
						}
						...
						if (currentEntryData.length != 0) {
							...
							this.refreshManifest(currentEntryData[currentEntryData.length - 1]);
						}
					} catch (ApplicationException e2) {
						Logger.defaultLogger().error(e2);
					} finally {
						this.application.disableWaitCursor();
					}
				} else {
					resetHistoryContent();
				}
