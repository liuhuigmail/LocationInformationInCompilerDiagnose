...
public class ListPane {
    ...
    private void refreshSelected() {
        ...
        int selected1 = menu.getSelectionIndex();
		if (selected1 != -1) {
		    ListPaneElement selectedElement =(ListPaneElement)elements.get(selected1);
		    selectedElement.getComposite().setVisible(true);
		    RowData dt1 = (RowData)selectedElement.getComposite().getLayoutData();
		    dt1.exclude = false;
		    if (panes.getSize().x > 3 && panes.getSize().y > 3) {
		        dt1.width = panes.getSize().x - 3;
		        dt1.height = panes.getSize().y - 3;
		    }
		    selectedElement.getComposite().setLayoutData(dt1);
		}
