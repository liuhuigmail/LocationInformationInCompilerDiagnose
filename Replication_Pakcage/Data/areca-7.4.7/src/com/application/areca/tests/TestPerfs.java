...
public class TestPerfs {
	...
	private static void cleanArchives(WorkspaceItem item) throws Exception {
		System.out.println("Destroying storage directories ...");
		WorkspaceProcessor.process(item, new TargetHandler() {
			public void handle(FileSystemTarget target) throws Exception {
	...
	private static void doBackup(final WorkspaceItem item, final String backupScheme) throws Exception {
		WorkspaceProcessor.process(item, new TargetHandler() {
			public void handle(FileSystemTarget target) throws Exception {
				...
				TransactionHandler transactionHandler = new YesTransactionHandler();
				...
				try {
					...
					target.processBackup(null, backupScheme, checkParams, transactionPoint, buildContext(target));
				} finally {
				...
				TransactionHandler transactionHandler1 = new YesTransactionHandler();
				try {
					LogHelper.logTarget(target);
					TransactionPoint transactionPoint = null;
				
					if (target.checkResumeSupported() == null && transactionHandler1 != null) {
						if (transactionHandler1.shallSearchForPendingTransactions()) {
							// Look for pending transactions
							Logger.defaultLogger().info("Looking for pending transactions ...");
							try {
								transactionPoint = target.getLastTransactionPoint(backupScheme);
							...
							if (transactionPoint != null) {
								Logger.defaultLogger().info("Transaction point found : " + transactionPoint.displayedName());
								if (transactionHandler1.shallHandleTransactionPoint(transactionPoint)) {
									String tpVersion;
									try {
										tpVersion = transactionPoint.readHeader().getArecaVersion();
				...
				TransactionHandler transactionHandler2 = new YesTransactionHandler();
				try {
					LogHelper.logTarget(target);
					TransactionPoint transactionPoint = null;
				
					if (target.checkResumeSupported() == null && transactionHandler2 != null) {
						if (transactionHandler2.shallSearchForPendingTransactions()) {
							// Look for pending transactions
							Logger.defaultLogger().info("Looking for pending transactions ...");
							try {
								transactionPoint = target.getLastTransactionPoint(backupScheme);
							...
							if (transactionPoint != null) {
								Logger.defaultLogger().info("Transaction point found : " + transactionPoint.displayedName());
								if (transactionHandler2.shallHandleTransactionPoint(transactionPoint)) {
									String tpVersion;
									try {
										tpVersion = transactionPoint.readHeader().getArecaVersion();
	...
	public static void main(String[] args) {
		...
		try {
			...
			final WorkspaceItem item = workspace;
			File filtered = new File(sources, CreateData.FILTERED_DIR);
			final String path = FileSystemManager.getAbsolutePath(filtered);
			WorkspaceProcessor.process(item, new TargetHandler() {
				public void handle(FileSystemTarget target) throws Exception {
