...
public class EncryptionConfiguration {
	...
	static {
		...
		AESParam.setTransformation(transformation);
		AESParam.setAlgorithm("AES");
		AESParam.setId(AES_HASH);
		AESParam.setIV(null);
		AESParam.setKeyConvention(KEYCONV_HASH);
		if (AESParam.isSupported()) {
			DEFAULT_PARAMETERS.put(AESParam.getId(), AESParam);
		}
		EncryptionConfiguration AESParam1 = new EncryptionConfiguration();
		AESParam1.setKeySize(16);
		String transformation1 = "AES";
		if (null != null && null != null) {
			transformation1 += "/" + null + "/" + null;
		}
		AESParam1.setTransformation(transformation1);
		AESParam1.setAlgorithm("AES");
		AESParam1.setId(AES_RAW);
		AESParam1.setIV(null);
		AESParam1.setKeyConvention(KEYCONV_RAW);
		if (AESParam1.isSupported()) {
			DEFAULT_PARAMETERS.put(AESParam1.getId(), AESParam1);
		}
