...
public class DeltaEntriesDispatcher extends AbstractEntriesDispatcher {
	...
	public void dispatchEntry(String entry) throws IOException {
		if (! initialized) {
			...
			for (int i1=0; i1<archives.length; i1++) {
				citers[i1] = ArchiveContentAdapter.buildIterator(ArchiveContentManager.resolveContentFileForArchive(this.medium, archives[i1]));
				titers[i1] = ArchiveTraceAdapter.buildIterator(ArchiveTraceManager.resolveTraceFileForArchive(this.medium, archives[i1]));
			}
		}
