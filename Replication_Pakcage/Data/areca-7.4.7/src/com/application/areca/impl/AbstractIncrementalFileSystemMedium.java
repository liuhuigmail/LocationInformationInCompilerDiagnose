...
implements TargetActions {
	...
	private EntryArchiveData entryarchivedata;
		...
		try {
			...
			if (! image) { // No merge if "overwrite" = true
				...
				if (recoveredFiles.length >= 2) {
					...
					File targetDirectory = FileSystemManager.getParentFile(target);
					FileTool r = FileTool.getInstance();
					if (!FileSystemManager.exists(targetDirectory)) {
						r.createDir(targetDirectory);
					}
					
					File tf = new File(targetDirectory, FileSystemManager.getName(target));
					OutputStream outStream = FileSystemManager.getFileOutputStream(tf,
							false, null);
					
					r.copyFile(traceFile, outStream, true, context.getTaskMonitor());
	...
	protected File duplicateMetadataFile(File source, ProcessContext context) {
		...
		if (FileSystemManager.exists(source)) {
			try {
				...
				r.copyFile(source, outStream, true, null);
			} catch (IOException e) {
				Logger.defaultLogger().error(e);
