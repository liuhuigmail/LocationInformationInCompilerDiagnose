...
public class FTPProxy extends AbstractProxy {
		...
		try {
			try {
				...
				if (this.client != null && this.client.isConnected()) {
					...
					this.debug("disconnect : disconnect");
				...
				Logger.defaultLogger().error("An error occurred while trying to disconnect from the following FTP server : " + this.remoteServer, e1);
			}
		} catch (Throwable ex) {
	...
	public synchronized boolean deleteFile(String remoteFile) throws FTPConnectionException {
		...
		if (this.client != null && this.client.isConnected()) {
			try {
				if ((System.currentTimeMillis() - this.lastOpTime) >= TIME_BETWEEN_OPS) {
					shallReconnect = ! this.client.sendNoOp();
					this.debug("checkConnection : noop", this.client.getReplyString());
					this.updateOpTime();
					shallReconnect = false;
					this.debug("checkConnection : connection successfully checked");
				} else {
					shallReconnect = false;
					this.debug("checkConnection : no need to check connection");
				}
			} catch (Throwable e1) {
				this.debug("checkConnection", e1);
				Logger.defaultLogger().error("Got an error during connection check", e1);
			}
		}
		...
		if (shallReconnect) {
			if (this.client != null) {
				Logger.defaultLogger().info("Disconnected from server : " + this.remoteServer + " ... tyring to reconnect.");
				this.debug("checkConnection : disconnected ... trying to reconnect", this.client.getReplyString());                
			} else {
				this.debug("checkConnection : not connected ... trying to connect"); 	
			}
			this.connect();
		}
		try {
	...
	public synchronized void completePendingCommand(boolean blocking) throws IOException, RemoteConnectionException {
		...
		if (! this.client.completePendingCommand()) {
			if (blocking) {
				throw new FTPConnectionException("Error trying to complete pending FTP instructions - got the following response from server : " + this.client.getReplyString().replace('\n', ' ').replace('\r', ' '));
			} else {
