...
public class SequenceAdapter implements Constants {
    ...
    public void serialize(OutputStream out, HashSequence sequence) throws IOException {
        out.write((int)(((long) VERSION >>>  0) & 0xff));
		out.write((int)((VERSION >>>  8) & 0xff));
    ...
    public byte[] serialize(HashSequence sequence) {
    	...
    	try {
	    	...
	    	try {
	    		zout.write((int)(((long) VERSION >>>  0) & 0xff));
				zout.write((int)((VERSION >>>  8) & 0xff));
				IOHelper.writeLong(sequence.getBlockSize(), zout);
				SimilarEntrySet[] sets = sequence.getInternalData();
				for (int i=0; i<sets.length; i++) {
					if (sets[i] != null) {
				        Iterator iter = sets[i].iterator();
				        while(iter.hasNext()) {
				            HashSequenceEntry entry = (HashSequenceEntry)iter.next();
				            IOHelper.writeInt(entry.getQuickHash(), zout);
				            IOHelper.writeLong(entry.getIndex(), zout);
				            IOHelper.writeInt(entry.getSize(), zout);
				            zout.write(entry.getFullHash());
				        }
					}
				}
	    	} finally {
