...
public class FileMutator {
	...
	public static void main(String[] args) {
		try {
			...
			String s = "Source : " + FileSystemManager.getAbsolutePath(source);
	...
	public static void processFiles(File source, File dest) throws Exception {
		if (ostream != null) {
			ostream.write(("Mutating " + FileSystemManager.getAbsolutePath(source) + " to " + FileSystemManager.getAbsolutePath(dest) + "\n").getBytes());
		}
	...
	private static byte[] mutate(byte[] in, int len, double probability, long position) throws IOException {
		...
		if (Math.abs(Util.getRnd()) < probability) {
			if (len > 2) {
				if (ostream != null) {
					ostream.write(("   Mutating byte at position " + position + "\n").getBytes());
				}
