...
implements ZipConstants {
        ...
        case DEFLATED:
            ...
            if (len == -1) {
				...
				if ((entry.flag & 8) == 8) {
				    ...
				    } else if (! entry.isZip64) {
				        entry.crc = get32(tmpbuf, EXTCRC);
				        entry.csize = get32(tmpbuf, EXTSIZ);
				        entry.setSize(get32(tmpbuf, EXTLEN));
				    } else {
				        entry.crc = get32(tmpbuf, EXTCRC);
				        entry.csize = get64(tmpbuf, EXTSIZ);
				        entry.setSize(get64(tmpbuf, EXTLEN64));
				    }
				}
				if ((ZIP_ENABLE_ENTRY_CHECK) && (entry.crc != crc.getValue())) {
				    throw new ZipException(
				            "invalid entry CRC (expected 0x" + Long.toHexString(entry.crc) +
				            " but got 0x" + Long.toHexString(crc.getValue()) + ")");
				}
				if (ZIP_ENABLE_ENTRY_CHECK && (entry.getSize() != getTotalOut())) {
					long mod = ((long)Integer.MAX_VALUE) - ((long)Integer.MIN_VALUE) + 1;
					long mv = (getTotalOut() - entry.getSize())%mod;
					if (mv != 0) {  // Backward compatibility
				        throw new ZipException(
				                "invalid entry size (expected " + entry.getSize() + " but got " +
				                getTotalOut() + " bytes)");
					}
				}
                entryEOF = true;
    ...
    private ZipEntry readLOC() throws IOException {
        ...
        long lc = get16(tmpbuf, 0) | ((long)get16(tmpbuf, 0+2) << 16); 
        if (lc != LOCSIG) {
            return null;
        }
