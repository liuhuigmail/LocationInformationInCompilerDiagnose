...
implements ZipConstants {
	...
	public void finish() throws IOException {
		try {
			...
			while (e.hasNext()) {
				ZipEntry e1 = (ZipEntry)e.next();
				byte[] nameBytes = ZipStringEncoder.encode(e1.getName(), charset);
				byte[] commentBytes;
				if (e1.getComment() != null) {
					commentBytes = ZipStringEncoder.encode(e1.getComment(), charset);
				} else {
					commentBytes = null;
				}
				...
				writeShort(e1.version);	    // version made by
				writeShort(e1.version);	    // version needed to extract
				writeShort(e1.flag);	    // general purpose bit flag
				writeShort(e1.getMethod());	    // compression method
				writeInt(e1.time);	    // last modification time
				writeInt(e1.getCrc());	    // crc-32
				if (useZip64) {
					writeInt(ZIP64SIZEFLAG);	    // compressed size
					writeInt(ZIP64SIZEFLAG);	    // uncompressed size
				} else {
					writeInt(e1.csize);      // compressed size
					writeInt(e1.getSize());       // uncompressed size
				}
				...
				if (useZip64) {
					...
					writeShort(e1.volumeNumber);         // starting disk number   
				}
				...
				if (useZip64) {
					...
					writeInt(e1.offset);     // relative offset of local header
				}
				writeBytes(nameBytes, 0, nameBytes.length);
				if (useZip64) {
					writeZip64ExtraField(e1);
				}
				if (commentBytes != null) {
	...
	private void writeLOC(ZipEntry e) throws IOException {
		...
		int size = SIZE_LOC + nameBytes.length;
		if (this.volumeStrategy != null) {
			((VolumeOutputStream)this.out).ensureCapacity((long) size);
		}
		long mark = this.totalWritten;
	...
	private void writeZip64ExtraField(ZipEntry e) throws IOException {
		...
		out.write((ZIP64XTRAFIELD >>> 0) & 0xff);
		out.write((ZIP64XTRAFIELD >>> 8) & 0xff);
		totalWritten += 2;
	...
	private void writeZip64END(long off, long len) throws IOException {       
		...
		OutputStream out1 = this.out;
		out1.write((int)((ZIP64ENDSIG >>>  0) & 0xff));
		out1.write((int)((ZIP64ENDSIG >>>  8) & 0xff));
		out1.write((int)((ZIP64ENDSIG >>> 16) & 0xff));
		out1.write((int)((ZIP64ENDSIG >>> 24) & 0xff));
		totalWritten += 4;	    		                   // zip64 end of central dir signature
	...
	private void writeEND(long off, long cenSize) throws IOException {
		...
		if (useZip64) {
			...
			writeLong(this.getEntryCount()); 			                   // total number of entries in the central directory on this disk
			writeLong(entries.size()); 			                   // total number of entries in the central directory
			writeLong(cenSize); 							                   // size of the central directory
			writeLong(off);  							                   // offset of start of central directory with respect to the starting disk number
