...
public class ZipStringEncoder {
	...
	public static byte[] encode(String s, Charset charset) {
		...
		String fromEncoded = decode(ret, 0, ret.length, charset);
		if (! fromEncoded.equals(s)) {
			Logger.defaultLogger().warn("Caution : unable to encode \"" + s + "\" properly using charset \"" + charset.name() + "\". It is advisable to use \"UTF-8\" as encoding for your zip archives.");
		}
		return ret;
	}
