...
public class FileTool {
	...
	public void copy(File sourceFileOrDirectory, File targetParentDirectory,
		...
		if (FileSystemManager.isFile(sourceFileOrDirectory)) {
			if (policy == null
				...
				File tf = new File(targetParentDirectory, FileSystemManager.getName(sourceFileOrDirectory));
				OutputStream outStream = FileSystemManager.getFileOutputStream(tf,
						false, listener);
				
				InputStream in;
				try {
					in = FileSystemManager.getFileInputStream(sourceFileOrDirectory);
				} catch (IOException e) {
					if (true) {
						try {
							outStream.close();
				...
				this.copy(in, outStream, true, true, monitor);
			}
		} else {
			...
			String[] files = FileSystemManager.list(sourceFileOrDirectory);
			if (files == null) {
				if (FileSystemManager.exists(sourceFileOrDirectory)) {
					Logger.defaultLogger().warn(
							FileSystemManager.getDisplayPath(sourceFileOrDirectory)
				...
				for (int i = 0; i < files.length; i++) {
					this.copy(new File(sourceFileOrDirectory, files[i]), td, policy, comparator, monitor, listener);
				}
			}
		}
	}
	...
	public String[] getFileRows(File sourceFile) throws IOException {
		return getInputStreamRows(FileSystemManager.getFileInputStream(sourceFile), null, -1, true);
	}
	...
	public boolean checkContains(File baseFile, String searchString)
			throws IOException {
		String content = this.getFileContent(baseFile, null);
