...
public class VMLauncher {
    ...
    public void run() {
		...
		command1.add(javaHome + "/bin/java");
		if (maxHeap != -1) {
		    command1.add("-Xmx" + maxHeap + "m");
		}
		if (initialHeap != -1) {
		    command1.add("-Xms" + initialHeap + "m");
		}
		command1.add("-cp");
		...
		while (transfered.hasNext()) {
		    String option = (String)transfered.next();
		    command1.add("-D" + option + "=" + System.getProperty(option));
		}
		...
		while (overriden.hasNext()) {
		    String option = (String)overriden.next();
		    String value = (String)overridenOptions.get(option);
		    command1.add("-D" + option + "=" + value);
		}
		
		command1.add(mainClass);
		
		for (int i=0; i<args.length; i++) {
		    command1.add(args[i]);
		}
		String[] command = (String[])command1.toArray(new String[command1.size()]);
    ...
    public static void main(String[] args) {
        ...
        lch.overridenOptions.put("file.encoding", "UTF-8");
