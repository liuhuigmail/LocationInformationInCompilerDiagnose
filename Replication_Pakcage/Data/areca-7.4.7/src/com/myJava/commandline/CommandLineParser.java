        ...
        while (position < argv.length) {
            ...
            if (current.startsWith("-")) { // handle -arg=value
                ...
                if (separator != -1) {
                    ...
                    CmdLineOption opt1 = (CmdLineOption) argMap.get(argName);
					if (opt1 == null)
					    throw new CmdLineParserException(
					            CmdLineParserException.UNKNOWN_OPTION, argName);
					CmdLineOption option = opt1;
					if (option != null) {
					    if (option.getType() == CmdLineOption.STRING) {
					        StringCmdLineOption stringOption = (StringCmdLineOption) option;
					        stringOption.setValue(argValue);
					    } else if (option.getType() == CmdLineOption.BOOLEAN) {
					        boolean bvalue = Boolean.valueOf(argValue).booleanValue();
					        BooleanCmdLineOption boolOption = (BooleanCmdLineOption) option;
					        boolOption.setValue(bvalue);
					    } else
					        throw new CmdLineParserException(
					                CmdLineParserException.TYPE_MISMATCH, argName);
					} else
					    throw new CmdLineParserException(
					            CmdLineParserException.UNKNOWN_OPTION, argName);
                } else
