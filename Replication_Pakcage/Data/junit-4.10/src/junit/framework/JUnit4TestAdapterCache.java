...
public class JUnit4TestAdapterCache extends HashMap<Description,Test> {
  ...
  public Test asTest(  Description description){
    if (description.isSuite())     return createTest(description);
 else {
      if (!containsKey(description))       put(description,createTest(description));
      return get(description);
    }
  }
  Test createTest(  Description description){
    if (description.isTest())     return new JUnit4TestCaseFacade(description);
 else {
  ...
  public List<Test> asTestList(  Description description){
    if (description.isTest())     return Arrays.asList(asTest(description));
 else {
